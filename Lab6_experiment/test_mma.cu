...
__global__ void wmma_gemm(half *a, half *b, float *c, int M, int N, int K) {
    ...
    for (int ki = 0; ki < K; ki += 16) {
        ...
        __half* addr_a = a + a_row * K + ki;
        __half* addr_b = b + b_col * K + ki;
...
void run_test(int M, int N, int K) {
    ...
    for (int i = 0; i < M * N; ++i) {
        ...
        if (diff > absolute_tolerance && rel_err > relative_tolerance) {
            if (error_count < 5) {
                std::cout << "WMMA error [" << i/N << "," << i%N << "] "
                          << "ref=" << basic_val << " got=" << wmma_val 
                          << " rel_err=" << rel_err*100 << "%" << std::endl;
            }
            wmma_valid = false;
            error_count++;
        }
    ...
    std::cout << "\nMatrix " << M << "x" << N << "x" << K << " results:\n"
              << "Tensor Core verify: " << (wmma_valid ? "PASS" : "FAIL") << "\n"
              << "Time: WMMA=" << time_wmma << "ms  Basic=" << time_basic << "ms\n"
              << "Speedup: " << time_basic/time_wmma << "x\n"
              << "Errors: " << error_count << "/" << M*N << std::endl;
...
int main() {
    ...
    for (int size : sizes) {
        if (size % 16 != 0) {
            std::cerr << "Skipping non-multiple-of-16 size: " << size << std::endl;
            continue;
        }
        try {
