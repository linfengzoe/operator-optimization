...
__global__ void implgemm(param_t param)
{
    ...
    // LDS addresses for reading from shared memory
    uint32_t weight_lds_addr = (warp_id / 2) * 32 + mma_tid_y * 4;
    uint32_t input_lds_addr  = (warp_id % 2) * 64 + mma_tid_x * 4;

    // Output tile coordinates
    int x = bx * 128 + input_lds_addr;   // Oh*Ow position in output
    int y = by * 128 + weight_lds_addr;  // K  position in output
    int z = blockIdx.z;                   // batch index

    // Shared memory: weight [8 CRS rows x 132 K cols], input [8 CRS rows x 128 OhOw cols]
    __shared__ DTYPE smemweight[8 * 132];
    __shared__ DTYPE smeminput[8 * 128];

    // STS addresses for storing to shared memory
    uint32_t weight_sts_addr = (tx % 8) * 132 + (tx / 8) * 4;
    uint32_t input_sts_addr  = (tx / 32) * 128 + (tx % 32);

    // Pre-compute input H/W base positions for this thread's Oh*Ow tile positions
    // Thread tx stores Oh*Ow positions: bx*128 + tx%32 + {0,32,64,96}
    int posOh_ori[4];
    int posOw_ori[4];
#pragma unroll
    for (int i = 0; i < 4; ++i)
    {
        ...
        posOh_ori[i] = (int)(ohow / param.Ow) * (int)param.u - (int)param.p;
        posOw_ori[i] = (int)(ohow % param.Ow) * (int)param.v - (int)param.q;
    }

    int outK      = by * 128 + (tx / 8) * 4;  // K index for this thread's weight load
    int weiKOffset = param.c * param.r * param.s;
    int weiCOffset = param.r * param.s;
    int inCOffset  = param.h * param.w;
    int inNOffset  = param.c * param.h * param.w;

    // Accumulation registers: 8 K values x 8 OhOw values
...
#pragma unroll
    ...
    // Main loop: iterate over C*R*S in steps of 8
    for (int crs = 0; crs < (int)(param.c * param.r * param.s); crs += 8)
    {
        // --- Load weight tile into smemweight ---
        // Each thread loads 4 K values for CRS row = (crs + tx%8)
        int curCRS = crs + tx % 8;
        int curC   = curCRS / (int)(param.r * param.s);
        int curR   = (curCRS % (int)(param.r * param.s)) / (int)param.s;
        int curS   = curCRS % (int)param.s;
...
#pragma unroll
        ...
        {
            int curK = outK + i;
            if (curK < (int)param.k && curCRS < (int)(param.c * param.r * param.s))
                weight_ldg_reg[i] = param.weight[curK * weiKOffset + curC * weiCOffset + curR * (int)param.s + curS];
            else
                weight_ldg_reg[i] = 0.0f;
        }

        // --- Load input tile into smeminput ---
        // Each thread loads 4 Oh*Ow positions for CRS row = (crs + tx/32)
        int curCRS2 = crs + (int)(tx / 32);
        int curC2   = curCRS2 / (int)(param.r * param.s);
        int curR2   = (curCRS2 % (int)(param.r * param.s)) / (int)param.s;
        int curS2   = curCRS2 % (int)param.s;

        DTYPE input_ldg_reg[4];
#pragma unroll
        for (int i = 0; i < 4; ++i)
        {
            int curH = posOh_ori[i] + curR2;
            int curW = posOw_ori[i] + curS2;
            if (curH >= 0 && curW >= 0 && curH < (int)param.h && curW < (int)param.w
                && curCRS2 < (int)(param.c * param.r * param.s))
                input_ldg_reg[i] = param.input[z * inNOffset + curC2 * inCOffset + curH * (int)param.w + curW];
            else
...
#pragma unroll
        ...
        // --- Compute outer products from shared memory ---
        DTYPE weight_frag[8];
        DTYPE input_frag[8];
#pragma unroll
...
#pragma unroll
    ...
    // Write output: output[n, k, oh, ow] stored as [N, K, Oh*Ow]
    int outNKOffset = param.k * param.Oh * param.Ow;
...
#pragma unroll
        for (int j = 0; j < 4; ++j)
        {
            outputOffset = z * outNKOffset + (y + i) * outKOffset + x + j;
            if (x + j < (int)(param.Oh * param.Ow) && y + i < (int)param.k)
                param.output[outputOffset] = output_frag[i][j];

            outputOffset = z * outNKOffset + (y + i) * outKOffset + x + j + 32;
            if (x + j + 32 < (int)(param.Oh * param.Ow) && y + i < (int)param.k)
                param.output[outputOffset] = output_frag[i][j + 4];

            outputOffset = z * outNKOffset + (y + i + 16) * outKOffset + x + j;
            if (x + j < (int)(param.Oh * param.Ow) && y + i + 16 < (int)param.k)
                param.output[outputOffset] = output_frag[i + 4][j];

            outputOffset = z * outNKOffset + (y + i + 16) * outKOffset + x + j + 32;
            if (x + j + 32 < (int)(param.Oh * param.Ow) && y + i + 16 < (int)param.k)
                param.output[outputOffset] = output_frag[i + 4][j + 4];
        }
    }
}
...
void conv2d_cuda_forward(param_t param)
{
    ...
    int thready = 1;
    int threadz = 1;
    int blockx = ((int)(param.Oh * param.Ow) + 127) / 128;
    int blocky = ((int)param.k + 127) / 128;
    int blockz = (int)param.n;
